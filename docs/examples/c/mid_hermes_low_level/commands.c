...
mid_hermes_ll_user_t* create_user(const char* user_id, const char* user_sk){
  ...
  assert(pk);
  if(0!=hermes_credential_store_get_public_key(cs, (const uint8_t*)user_id, strlen(user_id), &(pk->data), &(pk->length))){
  ...
  if(user_sk){
    ...
    return mid_hermes_ll_local_user_create(mid_hermes_ll_buffer_create((const uint8_t*)user_id, strlen(user_id)),
                                           mid_hermes_ll_buffer_create(ssk, ssk_length), pk);
  }
  return mid_hermes_ll_user_create(mid_hermes_ll_buffer_create((const uint8_t*)user_id, strlen(user_id)), pk);
}
...
int add_block(const char* user_id, const char* user_sk, const char* block_file_name, const char* block_meta_data){
  ...
  mid_hermes_ll_block_t* bl=mid_hermes_ll_block_create(u,
                                                       mid_hermes_ll_buffer_create((const uint8_t*)block_file_name, strlen(block_file_name)),
                                                       data,
                                                       mid_hermes_ll_buffer_create((const uint8_t*)block_meta_data, strlen(block_meta_data)),
                                                       NULL,
                                                       NULL);
  if(!bl){
...
int upd_block(const char* user_id, const char* user_sk, const char* block_file_name, const char* block_meta_data){
  ...
  mid_hermes_ll_buffer_t* block_id=mid_hermes_ll_buffer_create((const uint8_t*)block_file_name, strlen(block_file_name));
  mid_hermes_ll_buffer_t* new_meta=mid_hermes_ll_buffer_create((const uint8_t*)block_meta_data, strlen(block_meta_data));
