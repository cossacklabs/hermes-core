...
BIN=./docs/examples/c/mid_hermes_low_level
SUCCESS=0
FAIL=1
...
# add block
echo "add block"
$BIN/hermes_client_ll add_block $USER_ID $PRIVATE_KEY $TEST_FILE "metadata" 1>/dev/null
if [ ! $? -eq $SUCCESS ]; then
    exit 1
fi
...
# read block
echo "read block"
$BIN/hermes_client_ll read_block $USER_ID $PRIVATE_KEY $TEST_FILE 1>/dev/null
if [ ! $? -eq $SUCCESS ]; then
    exit 1
fi
...
$BIN/hermes_client_ll update_block $USER_ID $PRIVATE_KEY $TEST_FILE "new metadata" 1>/dev/null
if [ ! $? -eq $SUCCESS ]; then
    exit 1
fi
...
$BIN/hermes_client_ll grant_read $USER_ID $PRIVATE_KEY $TEST_FILE $USER_ID2 1>/dev/null
if [ ! $? -eq $SUCCESS ]; then
    exit 1
fi
...
$BIN/hermes_client_ll update_block $USER_ID2 $PRIVATE_KEY2 $TEST_FILE "new metadata" 1>/dev/null
if [ $? -eq $SUCCESS ]; then
    exit 1
fi
...
$BIN/hermes_client_ll grant_update $USER_ID $PRIVATE_KEY $TEST_FILE $USER_ID2 1>/dev/null
if [ ! $? -eq $SUCCESS ]; then
    exit 1
fi
...
echo "revoke update"
$BIN/hermes_client_ll revoke_update $USER_ID $PRIVATE_KEY $TEST_FILE $USER_ID2 1>/dev/null
if [ ! $? -eq $SUCCESS ]; then
    exit 1
fi
...
echo "revoke read"
$BIN/hermes_client_ll revoke_read $USER_ID $PRIVATE_KEY $TEST_FILE $USER_ID2 1>/dev/null
if [ ! $? -eq $SUCCESS ]; then
    exit 1
fi
...
# delete block
echo "delete block"
$BIN/hermes_client_ll delete_block $USER_ID $PRIVATE_KEY $TEST_FILE 1>/dev/null
if [ ! $? -eq $SUCCESS ]; then
    exit 1
fi
