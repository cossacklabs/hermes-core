...
hm_data_store_service_t *hm_data_store_service_create(hm_rpc_transport_t *transport, hm_ds_db_t *db) {
    ...
    hm_data_store_service_t *service = calloc(sizeof(hm_data_store_service_t), 1);
    assert(service);
    if (!(service->s = hm_data_store_server_create(transport, db))) {
        free(service);
        return NULL;
    }
    return service;
...
uint32_t hm_data_store_service_destroy(hm_data_store_service_t **service) {
    ...
    hm_data_store_server_destroy(&((*service)->s));
    free(*service);
    *service = NULL;
    return HM_SUCCESS;
}
...
uint32_t hm_data_store_service_start(hm_data_store_service_t *service) {
    ...
    while (true) {
        ...
        //TODO logging
        if (HM_SUCCESS != hm_data_store_server_call(service->s)) {
            return HM_FAIL;
        }
    }
    return HM_FAIL;
