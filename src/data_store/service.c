...
hm_data_store_service_t* hm_data_store_service_create(hm_rpc_transport_t* transport, hm_ds_db_t* db){
  ...
  hm_data_store_service_t* s=calloc(sizeof(hm_data_store_service_t), 1);
  assert(s);
  if(!(s->s=hm_data_store_server_create(transport, db))){
    free(s);
    return NULL;
  }
  return s;
...
uint32_t hm_data_store_service_destroy(hm_data_store_service_t** s){
  ...
  hm_data_store_server_destroy(&((*s)->s));
  free(*s);
  *s=NULL;
  return HM_SUCCESS;
}
...
uint32_t hm_data_store_service_start(hm_data_store_service_t* s){
  ...
  while(true){
    ...
    //TODO logging
    if(HM_SUCCESS!=hm_data_store_server_call(s->s)){
      return HM_FAIL;
    }
  }
  return HM_FAIL;
