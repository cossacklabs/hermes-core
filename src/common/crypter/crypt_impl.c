...
int hm_crypter_impl_encrypt(
    ...
    themis_status_t res = themis_secure_message_wrap(
            crypter->priv_, crypter->priv_length_, public_key, public_key_length, data, data_length,
...
int hm_crypter_impl_decrypt(
    ...
    if (!crypter || !(crypter->priv_) || !public_key || !public_key_length || !encrypted_data ||
        !encrypted_data_length) {
        return HM_INVALID_PARAMETER;
    }
    themis_status_t res = themis_secure_message_unwrap(
            crypter->priv_, crypter->priv_length_, public_key, public_key_length, encrypted_data, encrypted_data_length,
            data, data_length);
    switch (res) {
        case THEMIS_BUFFER_TOO_SMALL:
            return HM_BUFFER_TOO_SMALL;
...
int hm_crypter_impl_encrypt_with_token(
        ...
        const uint8_t *data, const size_t data_length,
        uint8_t *encrypted_data, size_t *encrypted_data_length) {
    if (!crypter || !(crypter->priv_) || !public_key || !public_key_length || !token || !token_length || !data ||
        !data_length) {
        return HM_INVALID_PARAMETER;
    }
    ...
    if (THEMIS_SUCCESS != themis_secure_message_unwrap(
            crypter->priv_, crypter->priv_length_, public_key, public_key_length, token, token_length,
            token_key, &token_key_length)
        || token_key_length != HM_TOKEN_LENGTH) {
        return HM_INVALID_PARAMETER;
    }
    themis_status_t res = themis_secure_cell_encrypt_seal(
            token_key, token_key_length, NULL, 0, data, data_length, encrypted_data, encrypted_data_length);
    switch (res) {
        case THEMIS_BUFFER_TOO_SMALL:
            return HM_BUFFER_TOO_SMALL;
        case THEMIS_SUCCESS:
...
int hm_crypter_impl_encrypt_with_creating_token(
    ...
    themis_status_t res = themis_secure_message_wrap(
            crypter->priv_, crypter->priv_length_, public_key, public_key_length, token_key, token_key_length,
            token, token_length);
    switch (res) {
        case THEMIS_BUFFER_TOO_SMALL:
            themis_secure_cell_encrypt_seal(
                    token_key, token_key_length, NULL, 0, data, data_length, NULL, encrypted_data_length);
            return HM_BUFFER_TOO_SMALL;
        case THEMIS_SUCCESS:
...
int hm_crypter_impl_decrypt_with_token(
        ...
        const uint8_t *encrypted_data, const size_t encrypted_data_length,
        uint8_t *data, size_t *data_length) {
    if (!crypter || !(crypter->priv_) || !public_key || !public_key_length || !token || !token_length ||
        !encrypted_data || !encrypted_data_length) {
        return HM_INVALID_PARAMETER;
    }
    uint8_t token_key[HM_TOKEN_LENGTH];
    ...
    themis_status_t res = themis_secure_cell_decrypt_seal(
            token_key, token_key_length, NULL, 0, encrypted_data, encrypted_data_length, data, data_length);
    switch (res) {
        case THEMIS_BUFFER_TOO_SMALL:
            return HM_BUFFER_TOO_SMALL;
        case THEMIS_SUCCESS:
...
int hm_crypter_impl_mac_with_token(
    ...
    if (THEMIS_BUFFER_TOO_SMALL != themis_secure_cell_encrypt_context_imprint(
            token_key, token_key_length, data, data_length, (uint8_t *) HM_DEFAULT_CTX, HM_DEFAULT_CTX_LENGTH,
            pre_mac, &pre_mac_length)) {
        return HM_FAIL;
    }
    ...
    switch (res) {
        case THEMIS_BUFFER_TOO_SMALL:
            *mac_length = HM_MAC_LENGTH;
            return HM_BUFFER_TOO_SMALL;
        case THEMIS_SUCCESS:
            return hm_crypter_impl_mac_with_token(
...
int hm_crypter_impl_create_token_from_token(
    ...
    if (!crypter || !crypter->priv_ || !public_key || !public_key_length || !new_public_key || !new_public_key_length ||
        !token || !token_length) {
        return HM_INVALID_PARAMETER;
    }
    uint8_t token_key[HM_TOKEN_LENGTH];
    ...
    themis_status_t res = themis_secure_message_wrap(
            crypter->priv_, crypter->priv_length_, new_public_key, new_public_key_length, token_key, token_key_length,
