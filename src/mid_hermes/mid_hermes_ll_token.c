...
mid_hermes_ll_token_t* mid_hermes_ll_token_generate(const mid_hermes_ll_buffer_t* id,
  ...
  HERMES_CHECK_IN_PARAM(id);
  HERMES_CHECK_IN_PARAM(user_and_owner);
  uint8_t rtoken[HM_TOKEN_LEN];-
  if(SOTER_SUCCESS!=soter_rand(rtoken, HM_TOKEN_LEN)){
    mid_hermes_ll_token_destroy(&t);
    return NULL;
  }
  uint32_t res;
  mid_hermes_ll_buffer_t* enc_token=mid_hermes_ll_buffer_create(NULL, 0);
  res=hm_asym_encrypt(user_and_owner->sk->data, user_and_owner->sk->length, user_and_owner->pk->data, user_and_owner->pk->length, token, HM_TOKEN_LEN, &(enc_token->data), &(enc_token->length));
  if(HM_SUCCESS!=res){
    mid_hermes_ll_token_destroy(&t);
    return NULL;
  }
...
mid_hermes_ll_token_t* mid_hermes_ll_token_get_token_for_user(mid_hermes_ll_token_t* t,
  ...
  HERMES_CHECK_IN_PARAM(t);
  HERMES_CHECK_IN_PARAM(for_user);
  mid_hermes_ll_buffer_t* b=mid_hermes_ll_token_get_token(t);
  if(!b){
    return NULL;
  }
  mid_hermes_ll_buffer_t* eb=mid_hermes_ll_buffer_create(NULL, 0);
  res=hm_asym_encrypt(t->user->sk->data, t->user->sk->length, for_user->pk->data, for_user->pk->length, b->data, b->length, &(eb->data), &(eb->length));
