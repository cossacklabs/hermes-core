...
hermes_user_t* hermes_user_create(const char* user_id, const uint8_t* private_key, const size_t private_key_length, hermes_storages_t* storages){
  ...
  HERMES_CHECK(0 == hermes_credential_store_get_pub_key(hermes_storages_get_credential_store(storages), user_id, &user->public_key, &user->public_key_length), hermes_user_destroy(&user); return NULL);
...
int hermes_user_save_block(hermes_user_t* user, hermes_document_block_t* block){
  ...
  HERMES_CHECK(0 == hermes_credential_store_get_pub_key(hermes_storages_get_credential_store(user->storages), block->owner_id, &owner_public_key, &owner_public_key_length),  return -1);
  ...
  if(block->is_loaded && block->is_changed){
    HERMES_CHECK(0== hermes_record_set_set_block(hermes_storages_get_record_set_store(user->storages), block->doc_id, block->id, block->mac, block->mac_length, blob, blob_length), block->data=old_data; block->data_length=old_data_length; free(encrypted_data); return -1);
  }else if(block->is_changed){
    HERMES_CHECK(0== hermes_record_set_set_new_block(hermes_storages_get_record_set_store(user->storages), block->doc_id, blob, blob_length, &(block->id)), block->data=old_data; block->data_length=old_data_length; free(encrypted_data); return -1);
    if(!(block->doc_id))
      block->doc_id = str_(block->id);
    HERMES_CHECK(0 == hermes_access_key_store_set_read_token(hermes_storages_get_access_key_store(user->storages), block->doc_id, block->id, user->id, block->read_token, block->read_token_length), block->data=old_data; block->data_length=old_data_length; free(encrypted_data); return -1);
    HERMES_CHECK(0 == hermes_access_key_store_set_update_token(hermes_storages_get_access_key_store(user->storages), block->doc_id, block->id, user->id, block->update_token, block->update_token_length), block->data=old_data; block->data_length=old_data_length; free(encrypted_data); return -1);
...
int hermes_user_load_block(hermes_user_t* user, const char* doc_id, const char* block_id, hermes_document_block_t** block){
  ...
  HERMES_CHECK(0 == hermes_record_set_get_block(hermes_storages_get_record_set_store(user->storages), doc_id, block_id, &blob, &blob_length), return -1);
  *block = hermes_document_block_create_from_blob(blob, blob_length);
  ...
  HERMES_CHECK(0 == hermes_credential_store_get_pub_key(hermes_storages_get_credential_store(user->storages), (*block)->owner_id, &owner_public_key, &owner_public_key_length), hermes_document_block_destroy(block); return -1);
  if((*block)->is_private){
    HERMES_CHECK(0 == hermes_access_key_store_get_read_token(hermes_storages_get_access_key_store(user->storages), doc_id, block_id, user->id, &((*block)->read_token), &((*block)->read_token_length)), hermes_document_block_destroy(block); return -1);
  ...
  if(0 == hermes_access_key_store_get_update_token(hermes_storages_get_access_key_store(user->storages), doc_id, block_id, user->id, &((*block)->update_token), &((*block)->update_token_length))){
...
int hermes_user_delete_block(hermes_user_t* user, hermes_document_block_t* block){
  ...
  HERMES_CHECK(0== hermes_record_set_delete_block(hermes_storages_get_record_set_store(user->storages), block->doc_id, block->id,  block->mac, block->mac_length), return -1);
  return 0;
}
...
int hermes_user_grand_access_to_block(hermes_user_t* user, const char* user_id_granting_access_to, hermes_document_block_t* block, int rights_mask){
  ...
  HERMES_CHECK(0 == hermes_credential_store_get_pub_key(hermes_storages_get_credential_store(user->storages), block->owner_id, &owner_public_key, &owner_public_key_length), return -1);  
  HERMES_CHECK(0 == hermes_credential_store_get_pub_key(hermes_storages_get_credential_store(user->storages), user_id_granting_access_to, &user_granting_to_public_key, &user_granting_to_public_key_length), free(owner_public_key);return -1);
  ...
  case HERMES_UPDATE_ACCESS_MASK:
    ...
    HERMES_CHECK(0==hermes_access_key_store_get_update_token(hermes_storages_get_access_key_store(user->storages), block->doc_id, block->id, user->id, &(block->update_token), &(block->update_token_length)), free(owner_public_key); free(user_granting_to_public_key); return -1);
    ...
    HERMES_CHECK(0 == hermes_access_key_store_set_update_token(hermes_storages_get_access_key_store(user->storages), block->doc_id, block->id, user_id_granting_access_to, new_token, new_token_length), free(owner_public_key); free(user_granting_to_public_key); free(new_token); return -1);
    free(new_token);
  case HERMES_READ_ACCESS_MASK:
    free(block->read_token);
    block->read_token=NULL;
    HERMES_CHECK(0==hermes_access_key_store_get_read_token(hermes_storages_get_access_key_store(user->storages), block->doc_id, block->id, user->id, &(block->read_token), &(block->read_token_length)), free(owner_public_key); free(user_granting_to_public_key); return -1);
    if(block->read_token_length){
      ...
      HERMES_CHECK(0 == hermes_access_key_store_set_read_token(hermes_storages_get_access_key_store(user->storages), block->doc_id, block->id, user_id_granting_access_to, new_token, new_token_length), free(owner_public_key); free(user_granting_to_public_key); free(new_token); return -1);
      free(new_token);
    }
    break;
  default:
...
int hermes_user_deny_access_to_block(hermes_user_t* user, const char* user_id_denying_access_to, hermes_document_block_t* block){
  HERMES_CHECK(user && user->crypter && block && block->is_loaded && block->mac && block->mac_length, return -2);
  HERMES_CHECK(0==hermes_access_key_store_rem_read_token(hermes_storages_get_access_key_store(user->storages), block->doc_id, block->id, user->id, user_id_denying_access_to), return -1);
  HERMES_CHECK(0==hermes_access_key_store_rem_update_token(hermes_storages_get_access_key_store(user->storages), block->doc_id, block->id, user->id, user_id_denying_access_to), return -1);
  return 0;
}
