...
hermes_crypter_t* hermes_crypter_create(const uint8_t* private_key, const size_t private_key_length){
  ...
  HERMES_CHECK(-1 == pipe(crypter->crypt_reader_pipe), hermes_crypter_destroy(&crypter); return NULL);
  HERMES_CHECK(-1 == pipe(crypter->crypt_writer_pipe), hermes_crypter_destroy(&crypter); return NULL);
  ...
  if(0 == crypter->crypter_pid){
    ...
    uint8_t* buf;
    while(true){
      HERMES_CHECK(sizeof(uint32_t) == read(crypter->crypt_reader_pipe[0], &command, sizeof(uint32_t)), break);
      ...
      case EXIT:
	command = EXITED;
	HERMES_CHECK(sizeof(uint32_t)==write(crypter->crypt_writer_pipe[1], &command, sizeof(uint32_t)), error=true; break);
	error=true;
	break;
      case ENCRYPT:
	HERMES_CHECK(read_data_from_pipe(crypter->crypt_reader_pipe[0], &key, (uint32_t*)(&key_length)), error=true; break);
	HERMES_CHECK(read_data_from_pipe(crypter->crypt_reader_pipe[0], &data, (uint32_t*)(&data_length)), free(key); error=true; break);
	HERMES_CHECK(THEMIS_BUFFER_TOO_SMALL == themis_secure_message_wrap(private_key, private_key_length, key, key_length, data, data_length, NULL, &res_data_length), free(key); free(data); error=true; break);
	res_data=malloc(res_data_length);
	HERMES_CHECK(res_data && THEMIS_SUCCESS == themis_secure_message_wrap(private_key, private_key_length, key, key_length, data, data_length, res_data, &res_data_length), free(key); free(data); free(res_data); error=true; break);
	free(key);
	free(data);
	HERMES_CHECK(write_data_to_pipe(crypter->crypt_writer_pipe[1], res_data, (uint32_t)res_data_length), free(res_data); error=true; break);
	free(res_data);
	break;
      case DECRYPT:
	HERMES_CHECK(read_data_from_pipe(crypter->crypt_reader_pipe[0], &key, (uint32_t*)(&key_length)), error=true; break);
	HERMES_CHECK(read_data_from_pipe(crypter->crypt_reader_pipe[0], &data, (uint32_t*)(&data_length)), free(key); error=true; break);
	HERMES_CHECK(THEMIS_BUFFER_TOO_SMALL == themis_secure_message_unwrap(private_key, private_key_length, key, key_length, data, data_length, NULL, &res_data_length), free(key); free(data); error=true; break);
	res_data=malloc(res_data_length);
	HERMES_CHECK(res_data && THEMIS_SUCCESS == themis_secure_message_unwrap(private_key, private_key_length, key, key_length, data, data_length, res_data, &res_data_length), free(key); free(data); free(res_data); error=true; break);
	free(key);
	free(data);
	HERMES_CHECK(write_data_to_pipe(crypter->crypt_writer_pipe[1], res_data, (uint32_t)res_data_length), free(res_data); error=true; break);
	free(res_data);
