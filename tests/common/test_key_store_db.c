...
uint32_t db_set_rtoken(
  ...
  assert(node_token);
  memcpy(node_token->user_id, owner_id, USER_ID_LENGTH);
  ...
  searched_token = *(hm_test_ks_db_token_t**)(tsearch)(node_token, &(serached_block_node->users), hm_ks_test_db_token_compare);
...
uint32_t db_set_wtoken(
  ...
  memcpy(node_token->user_id, user_id, USER_ID_LENGTH);
  searched_token= *(hm_test_ks_db_token_t**)(tfind)(node_token, &(serached_block_node->users), hm_ks_test_db_token_compare);
  if(!searched_token){
    free(node_token);
    return HM_FAIL;
  }
...
uint32_t db_del_rtoken(
  ...
  free(token);
  token= *(hm_test_ks_db_token_t**)(tdelete)(searched_token, &(serached_block_node->users), hm_ks_test_db_token_compare);
  free(searched_token);
  return HM_SUCCESS;
}
