...
//simple storage can store only 10 different key_pairs
#define HM_SIMPLE_KEY_STORAGE_CAPACITY 10


typedef struct {
    uint8_t id[HM_SIMPLE_KEY_STORAGE_ID_LENGTH];
    uint8_t private_key[HM_SIMPLE_KEY_STORAGE_PRIVATE_KEY_LENGTH];
    uint8_t public_key[HM_SIMPLE_KEY_STORAGE_PUBLIC_KEY_LENGTH];
...
hm_keys_storage_t *hm_keys_storage_create() {
    ...
    while (!feof(file) && !ferror(file) && (store->count_ < HM_SIMPLE_KEY_STORAGE_CAPACITY)) {
        ...
        if (readed < HM_SIMPLE_KEY_STORAGE_ID_LENGTH && !(store->count_)) {
            free(store);
        ...
        if (readed < HM_SIMPLE_KEY_STORAGE_PRIVATE_KEY_LENGTH && !(store->count_)) {
            free(store);
        ...
        if (readed < HM_SIMPLE_KEY_STORAGE_PUBLIC_KEY_LENGTH && !(store->count_)) {
            free(store);
...
int hm_keys_storage_get_key(
        hm_keys_storage_t *key_store, const uint8_t *id, const size_t id_length, uint8_t *key, size_t *key_length,
        bool is_private) {
    if (!key_store || !id || (id_length != HM_SIMPLE_KEY_STORAGE_ID_LENGTH)) {
        return HM_INVALID_PARAMETER;
    }
    int i;
    for (i = 0; i < HM_SIMPLE_KEY_STORAGE_CAPACITY; ++i) {
        if (0 == memcmp(id, key_store->nodes_[i].id, HM_SIMPLE_KEY_STORAGE_ID_LENGTH)) {
            break;
        }
    }
    if (HM_SIMPLE_KEY_STORAGE_CAPACITY == i ||
        (0 == (is_private ? (key_store->nodes_[i].private_key[0]) : (key_store->nodes_[i].public_key[0])))) {
        return HM_FAIL;
    }
    if (!key || ((*key_length) <
                 (is_private ? HM_SIMPLE_KEY_STORAGE_PRIVATE_KEY_LENGTH : HM_SIMPLE_KEY_STORAGE_PUBLIC_KEY_LENGTH))) {
    ...
    memcpy(key, is_private ? (key_store->nodes_[i].private_key) : (key_store->nodes_[i].public_key),
           is_private ? HM_SIMPLE_KEY_STORAGE_PRIVATE_KEY_LENGTH : HM_SIMPLE_KEY_STORAGE_PUBLIC_KEY_LENGTH);
    return HM_SUCCESS;
}

int hm_keys_storage_get_key_(hm_keys_storage_t *key_store, const uint8_t *id, const size_t id_length, uint8_t **key,
                             size_t *key_length, bool is_private) {
    size_t kl = 0;
    if (HM_BUFFER_TOO_SMALL != hm_keys_storage_get_key(key_store, id, id_length, NULL, &kl, is_private)) {
        return HM_FAIL;
    }
    *key = malloc(kl);
    if (!(*key)) {
    ...
    if (HM_SUCCESS != hm_keys_storage_get_key(key_store, id, id_length, *key, &kl, is_private)) {
        free(*key);
        *key = NULL;
        return HM_FAIL;
    }
...
int hm_keys_storage_get_private_key_by_id(
        hm_keys_storage_t *key_store, const uint8_t *id, const size_t id_length, uint8_t *key, size_t *key_length) {
    return hm_keys_storage_get_key(key_store, id, id_length, key, key_length, true);
}

int hm_keys_storage_get_public_key_by_id(
        hm_keys_storage_t *key_store, const uint8_t *id, const size_t id_length, uint8_t *key, size_t *key_length) {
    return hm_keys_storage_get_key(key_store, id, id_length, key, key_length, false);
}

int hm_keys_storage_get_private_key_by_id_(
        hm_keys_storage_t *key_store, const uint8_t *id, const size_t id_length, uint8_t **key, size_t *key_length) {
    return hm_keys_storage_get_key_(key_store, id, id_length, key, key_length, true);
}

int hm_keys_storage_get_public_key_by_id_(
        hm_keys_storage_t *key_store, const uint8_t *id, const size_t id_length, uint8_t **key, size_t *key_length) {
    return hm_keys_storage_get_key_(key_store, id, id_length, key, key_length, false);
}
